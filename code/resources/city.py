    ...
    def post(self):
        ...
        if existing_cities:
            ...
            if pincode in existing_pincodes:
                return {"message": f"'{city_name}' city with pincode '{pincode}' already exists."}
...
class CityQuery(Resource):
    ...
    def get(self, identifier):
        ...
        city = City.find_by_id(identifier) if is_digit else City.find_by_name(identifier)
        if city:
            return [c.to_json() for c in city] if isinstance(city, list) else city.to_json(), 200

        return {"message": f"No city with {'id' if is_digit else 'name'} '{identifier}' found!"}, 404
